...
const makeMessagesRecvSocket = (config) => {
    ...
    const sendRetryRequest = async (node, forceIncludeKeys = false) => {
        ...
        // ✅ Fix TS2365 by forcing retryCount to be number
        const retry = msgRetryCache.get(key);
        ...
        msgRetryCache.set(key, nextRetryCount);
        ...
        await authState.keys.transaction(async () => {
            ...
            if (nextRetryCount > 1 || forceIncludeKeys) {
            ...
            logger.info({ msgAttrs: node.attrs, retryCount: nextRetryCount }, 'sent retry receipt');
        });
    };
    ...
    const willSendMessageAgain = (id, participant) => {
        ...
        const retryCount = typeof msgRetryCache.get(key) === 'number' ? msgRetryCache.get(key) : 0;
        return retryCount < maxMsgRetryCount;
    };
    const updateSendMessageAgainCount = (id, participant) => {
        const key = `${id}:${participant}`;
        const current = msgRetryCache.get(key);
    ...
    const handleCall = async (node) => {
        ...
        // Cek apakah data cache ada dan valid
        const existingCall = callOfferCache.get(call.id);
        if (typeof existingCall === 'object' &&
            existingCall !== null &&
            'isVideo' in existingCall &&
            'isGroup' in existingCall) {
            const typedCall = existingCall;
            call.isVideo = typedCall.isVideo;
            call.isGroup = typedCall.isGroup;
        }
    ...
    const handleBadAck = async ({ attrs }) => {
        ...
        // current hypothesis is that if phash is sent in the ack
        // it means -- the message hasn't reached all devices yet
        // we'll retry sending the message here
        if (attrs.phash) {
            logger.info({ attrs }, 'received phash in ack, resending message...');
            const cacheKey = `${key.remoteJid}:${key.id}`;
            const retryVal = msgRetryCache.get(cacheKey);
            const retryCount = typeof retryVal === 'number' ? retryVal : 0;
            if (retryCount >= maxMsgRetryCount) {
